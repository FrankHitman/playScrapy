...
class NHKHLSDownloader:
    def __init__(self, m3u8_content, cookies_string, base_url=None):
        ...
        self.key_uri = None
        self.key_data = None
        default_referer = base_url if base_url and base_url.startswith('http') else 'https://news.web.nhk/news/easy/'
        self.headers = {
            'Accept': '*/*',
    ...
    def _download_segment(self, segment_name, output_dir):
        ...
        # 构建完整URL
        if self.base_url:
            segment_url = urljoin(self.base_url, segment_name)
        else:
            # 从M3U8内容推断基础URL
            segment_url = f"https://media.vd.st.nhk/news/easy_audio/ne2025101017047_4IoalMQ7I5LORUIVdT34ExOCPsoXNKeSeEYMwuSy/{segment_name}"
        
        # 添加必要的payload参数来避免403错误
        payload = {
            'hdntl': 'exp=1760267440~acl=/*~data=hdntl~hmac=d4c8373db47aa78e836a0364b328906a149cfef8477ab7af22754ac74c007aed',
            'aka_me_session_id': 'AAAAAAAAAACwjOtoAAAAAN6h5xsIs1%2fcA6%2fOJnHVxQNl4wn8CxHoVAcK5pkZmNzz5BKbsD1Od9hxf69Z+SyjTUp%2f6MsiCRwY',
            'aka_media_format_type': 'hls'
        }
        
        try:
            print(f"正在下载片段: {segment_name}")
            response = requests.get(segment_url, headers=self.headers, cookies=self.cookies, params=payload, timeout=30)
            response.raise_for_status()
...
#EXT-X-ENDLIST"""
    ...
    # 创建下载器
    downloader = NHKHLSDownloader(m3u8_content, cookies_string)
